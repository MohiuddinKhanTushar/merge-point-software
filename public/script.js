...
function loadActiveBids(orgId, userId, userRole) {
    ...
    if (userRole === 'admin' || userRole === 'manager') {
        bidsQuery = query(collection(firestore, "bids"), where("orgId", "==", orgId), where("status", "in", ["drafting", "review", "scoping", "approved"]));
    } else {
        bidsQuery = query(collection(firestore, "bids"), where("orgId", "==", orgId), where("ownerId", "==", userId), where("status", "in", ["drafting", "review", "scoping", "approved"]));
    }

    onSnapshot(bidsQuery, (snapshot) => {
        if (snapshot.empty) {
            bidsGrid.innerHTML = `<p>No active bids found.</p>`;
            return;
        }
            ...
            combinedHtml += `
                <div class="bid-card" id="${bidId}">
                    ...
                    <div class="progress-container-mini">
                        <div class="progress-label"><span>AI Content Ready</span><span>${bid.progress || 0}%</span></div>
                        <div class="progress-bar-mini"><div class="fill" style="width: ${bid.progress || 0}%;"></div></div>
                    </div>
                    <div style="display: flex; gap: 10px; margin-top: 1rem;">
                        ...
                        <button class="btn-secondary-outline" onclick="archiveBid('${bidId}')"><i data-lucide="archive"></i></button>
                        <button class="btn-secondary-outline" onclick="deleteBid('${bidId}')" style="color: #ef4444;"><i data-lucide="trash-2"></i></button>
                    </div>
                </div>
            `;
        });
...
async function checkMasterDocument(userId) {
    const q = query(collection(db, "knowledge"), where("ownerId", "==", userId), where("status", "in", ["ready", "processing"]));
    const snap = await getDocs(q);
    return !snap.empty; 
}

async function archiveBid(bidId) {
    const confirmed = await showConfirm("Archive Project?", "This will move it to your Library.", "Archive");
    if (confirmed) {
        const bidRef = doc(firestore, "bids", bidId);
        const bidSnap = await getDoc(bidRef);
        if (bidSnap.exists()) {
            await addDoc(collection(firestore, "archived_rfps"), { ...bidSnap.data(), dateArchived: serverTimestamp() });
...
async function deleteBid(bidId) {
    const confirmed = await showConfirm("Delete Permanently?", "This cannot be undone.", "Delete");
    if (confirmed) {
        const bidRef = doc(firestore, "bids", bidId);
        const bidSnap = await getDoc(bidRef);
        if (bidSnap.exists()) {
            ...
            if (data.fileName) {
                try { await deleteObject(ref(storage, `tenders/${auth.currentUser.uid}/${data.fileName}`)); } catch (e) {}
            }
            await deleteDoc(bidRef);
        }
    }
