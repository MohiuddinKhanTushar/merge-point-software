        ...
        if (user) {
            try {
                ...
                if (userSnap.exists()) {
                    ...
                    // Setup listeners with both orgId and userId for filtering
                    setupDataListeners(orgId, user.uid, userRole);
                }
            } catch (error) {
...
function setupDataListeners(orgId, userId, userRole) {
    // 1. Listen for Active Bids
    let activeQuery;
    if (userRole === 'admin' || userRole === 'manager') {
        activeQuery = query(collection(db, "bids"), where("orgId", "==", orgId));
    } else {
        activeQuery = query(collection(db, "bids"), where("orgId", "==", orgId), where("ownerId", "==", userId));
    }
    ...
    // 2. Listen for Archived RFPs (Historical Win/Loss)
    let archiveQuery;
    if (userRole === 'admin' || userRole === 'manager') {
        archiveQuery = query(collection(db, "archived_rfps"), where("orgId", "==", orgId));
    } else {
        archiveQuery = query(collection(db, "archived_rfps"), where("orgId", "==", orgId), where("ownerId", "==", userId));
    }
...
function processAnalytics(data) {
    ...
    if (winsList) {
        const recentWins = data.filter(item => item.outcome === 'won').slice(0, 4); 
        winsList.innerHTML = recentWins.length > 0 ? recentWins.map(win => `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 8px;">
                <div style="display: flex; flex-direction: column;">
                    <span style="font-weight: 600; font-size: 0.85rem; color: #1e293b;">${win.bidName || 'Untitled Bid'}</span>
                    <span style="font-size: 0.75rem; color: #64748b;">${win.client || 'N/A'}</span>
                </div>
                <span style="font-size: 0.7rem; padding: 4px 10px; background: #dcfce7; color: #166534; border-radius: 12px; font-weight: 700;">WON</span>
            </div>
        `).join('') : '<p style="font-size: 0.8rem; color: #94a3b8;">No records found.</p>';
    }
